...
Ibexa DXP v4.5 adds new features to Ibexa Commerce, translation comparison, and a number of improvements to Customer Portal and Personalization.
...
### All-new Ibexa Commerce packages (Commerce)
...
New screens added to the back office user interface let Ibexa DXP users search for orders and filter search results. Users can also review order details and completion status, and cancel orders.

![The order list screen](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_order_list.png "The order list screen")

#### Payment

The all-new Payment module brings a possibility of tracking payment progress and defining a custom payment processing workflow. New back office screens allow users to search for payment methods and payments, and also define, enable, and disable offline payment methods.

Additionally, new APIs are available, which can be used for managing payment methods and payments.

![The payment methods screen](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_payment_methods.png "The payment methods screen")

#### Shipping

With the arrival of the Shipping module, it's now possible to define and manage shipping methods of different types, together with their related costs, on a dedicated back office screen. You can now also configure and customize the shipment workflow.
...
![The shipping methods screen](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_shipping_methods.png "The shipping methods screen")

For more information, see [Commerce](https://doc.ibexa.co/en/4.5/commerce/commerce/).

### New commerce page blocks (Experience, Commerce)
...
![Bestsellers block](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_bestsellers_block.png "Bestsellers block")
...
![React app block](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_react_app_block.png "React app block")
...
![Translation comparison](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_comparison_view.png "Translation comparison")
...
### Page Builder for B2B portals (Experience, Commerce)

With this release, you're able to use Page Builder to create custom Customer Portals for your clients. With new Sales rep page block and using all available blocks from the original Page Builder, you can create a unique experience for each customer group. Additionally, you can assign each customer group to a specific Customer Portal or create an availability hierarchy based on rules and configuration.

![Page Builder for B2B portals](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_page_builder_b2b.png "Page Builder for B2B portals")

For more information, see [backend configuration](https://doc.ibexa.co/en/4.5/customer_management/cp_page_builder/) and [user guide](https://doc.ibexa.co/projects/userguide/en/4.5/customer_management/build_customer_portal/) on how to create and edit Customer Portals.
...
![Segment management](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_segment_management.png "Segment management logic")
...
In this release, the CDP configuration becomes more generic and allows supporting other transport types accepted by CDP. Currently, only `stream_file` transport is supported and can be initialized from the configuration.
...
#### PHP API for company accounts (Experience, Commerce)

To create a company with proper structure and shipping address by using PHP API, we recommend new `\Ibexa\Contracts\CorporateAccount\Service\CorporateAccountService::createCompany` service instead of `\Ibexa\Contracts\CorporateAccount\Service\CompanyService::createCompany`.

#### REST API for order management (Commerce)
...
#### PHP API for order management (Commerce)
...
#### PHP API for shipping methods and shipments (Commerce)
...
#### PHP API for payment methods and payments (Commerce)
...
![Product categories filter](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_product_categories.png "Product categories filter")
...
- `\Ibexa\AdminUi\Siteaccess\SiteaccessResolverInterface::getSiteaccessesForLocation` replaced by `\Ibexa\AdminUi\Siteaccess\SiteaccessResolverInterface::getSiteAccessesList`
...
![Product category tree filter](https://doc.ibexa.co/en/5.0/release_notes/img/4.5_product_category_tree.png "Product category tree filter")
