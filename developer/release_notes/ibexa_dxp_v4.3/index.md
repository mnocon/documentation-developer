...
Ibexa DXP v4.3 adds the improvements to the Customer Portal, product catalog and SEO.
...
For more information, see [Customer Portal applications documentation](../../customer_management/cp_applications/index.md).

![Company self-registration](https://doc.ibexa.co/en/5.0/release_notes/img/4.3_self_registration.png)

#### Customization of approval process

You can now [customize the approval process](../../customer_management/cp_applications/index.md#customization-of-an-approval-process) for company self-registration. By adding additional steps and options, you can build a process that perfectly meets your business needs.
...
- [`BasePrice` Sort Clause](../../search/sort_clause_reference/baseprice_sort_clause/index.md) sorts results by the products' base prices
- [`CustomPrice` Sort Clause](../../search/sort_clause_reference/customprice_sort_clause/index.md) enables sorting by the custom price configured for the provided customer group.

#### Usability improvements

This release also includes a number of usability improvements in the product catalog, such as full information about available attribute values or improved display of Selection attributes.

You can now move assets between collections by using drag and drop.

![Moving assets between collection with drag and drop](https://doc.ibexa.co/en/5.0/release_notes/img/4.3_collection_dnd.png)

From product's **Completeness** tab you can now jump directly to editing the product prices in all configured currencies.

![Editing product price from Completeness tab](https://doc.ibexa.co/en/5.0/release_notes/img/4.3_edit_product_price.png)

#### Catalog filters

In catalogs, you can now [configure default filters](https://doc.ibexa.co/en/4.3/pim/pim_configuration/#catalog-filters) that are always added to a catalog, define filter order, and group custom filters. Built-in filters are also divided into groups now for easier browsing.
...
This release brings you a new content type for private customers registering from the front page. We also prepared a migration command for already existing users to ease your upgrade process.
...
Now, as a Personalization admin, after editing a model in the back office, [you can build this model](https://doc.ibexa.co/projects/userguide/en/master/personalization/configure_models/#trigger-model-build), use the **Trigger model build** button to build this model with your modifications.
...
- You can now [customize Elasticsearch index structure](../../search/extensibility/customize_elasticsearch_index_structure/index.md) to manage how documents in the index are grouped.
- A new [`ibexa_seo_is_empty()` Twig function](../../templating/twig_function_reference/content_twig_functions/index.md#ibexa_content_name) checks whether SEO data is available for a content item.
